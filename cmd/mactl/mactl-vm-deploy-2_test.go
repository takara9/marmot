	...
	Context("様々に条件を変えてサーバーのデプロイ", func() {
		...
		It("仮想サーバーの状態確認 test-20,21,22", func() {
			for _, serverId := range []string{serverId_1, serverId_2, serverId_3} {
				...
				err = json.Unmarshal(stdoutStderr, &server)
				Expect(err).NotTo(HaveOccurred())
				Expect(server.Status.StatusCode).To(Equal(int(db.SERVER_RUNNING)), "server %s entered non-running state: code=%d, message=%s", serverId, server.Status.StatusCode, util.OrDefault(server.Status.Message, ""))
			}
		})
