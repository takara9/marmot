...
| `dns_client_allow_cidrs` | upstream DNS への転送を許可するクライアント CIDR 一覧 | `[]` |
...
- `dns_client_allow_cidrs` は、外部 DNS へのフォワードを許可するクライアントネットワークを明示する用途です。空の場合は upstream 転送を制限します。
...
- `session_idle_timeout`: セッション無通信タイムアウト（例: `30m`, `2h`, `1d`）。
  - 指定可能範囲は **1m〜7d（最大7日）** です。
  - `7d` を超える値（例: `10d`）は起動時に設定エラーになります。
