...
mactl コマンドに、新たな kubectlライクで、使いやすいコマンド体系を追加する。
...
- -l VERBが、getの時に有効で labelのキーと値で一致したものだけが表示される


## マニフェストと REST-API エンドポイントとの対応について

apiVersionにより、apiのバージョンを切り替える。
kindにより、mactl コマンドのエンドポイントのパスを切り替える。
