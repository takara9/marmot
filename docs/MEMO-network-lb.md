...
- APIの定義は、OpenAPIの定義ファイル api/marmot-api-v1.yaml に項目を追加して、Go言語のソースコードを生成して使用する。独自に構造体を作成してならない。
...
- 外部公開ポートは Ansible で iptablesを設定する
...
- LB VM のスケジュール先は仮想マシンのスケジューラーに委任し、NLBコントローラーは配置先を直接決定しない。
- VM 起動後、Ansible playbookを適用して、iptablesで設定を実施。
- Ansible playbookは、適用前にmarmotd の稼働するホストの/var/lib/marmot/ansible-playbooks に作成した上で、VMのOSへ適用する。
- Ansible playbookのファイル名は、ファイルの一意性を保証するために、NLBのIDを利用する。NLBの削除時は playbookのファイルも消す。
...
- 許可元は spec.remoteCIDR のみ通過。デフォルトは全解放
- 管理用鍵は /etc/marmot/keys 配下を利用する。
