...
var _ = Describe("IPAM", Ordered, func() {
	...
	Describe("IPアドレス管理テスト", Ordered, func() {
		...
		var vnetId string = "12345" // 仮想ネットワークIDの例
		Context("IPv4のテスト", func() {
			...
			It("IPv4ネットワークアドレスの作成 #1", func() {
				...
				Expect(err).NotTo(HaveOccurred())
				fmt.Println("作成されたIPv4ネットワークのID:", id)
				idIpv4_1 = id
			})

			It("KeyからIPv4ネットワーク情報を取得", func() {
				GinkgoWriter.Printf("IPv4ネットワークID: %s\n", id)
				net, err := v.GetIpNetworkById(vnetId, id)
				Expect(err).NotTo(HaveOccurred())
			...
			It("ネットワークの一覧取得", func() {
				nets, err = v.GetIpNetworks(vnetId)
				Expect(err).NotTo(HaveOccurred())
		...
		Context("IPv4アドレスの振り出し", func() {
			...
			It("IPアドレスの割り当て #1", func() {
				...
				Expect(err).NotTo(HaveOccurred())
				ip, mask, err := v.AllocateIP(vnetId, idIpv4_1, "host1")
				Expect(err).NotTo(HaveOccurred())
			...
			It("IPアドレスのリリース #1", func() {
				err = v.ReleaseIP(vnetId, idIpv4_1, ipaddr)
				Expect(err).NotTo(HaveOccurred())
			})
			...
			It("IPアドレスの割り当て #2", func() {
				for i := 2; i < 254; i++ {
					...
					ip, mask, err := v.AllocateIP(vnetId, idIpv4_1, hostname)
					Expect(err).NotTo(HaveOccurred())
					fmt.Printf("Allocated IP: %s/%d\n", ip, mask)
				}
			})
			...
			It("IPアドレスの割り当て #3", func() {
				ip, mask, err := v.AllocateIP(vnetId, idIpv4_1, "host255")
				Expect(err).To(HaveOccurred())
				fmt.Printf("Allocated IP: %s/%d\n", ip, mask)
			})

			It("IPアドレスの割り当て #4", func() {
				ip, mask, err := v.AllocateIP(vnetId, idIpv4_2, "host_b01")
				Expect(err).NotTo(HaveOccurred())
				fmt.Printf("Allocated IP: %s/%d\n", ip, mask)
			})

			It("IPアドレスの割り当て #5", func() {
				ip, mask, err := v.AllocateIP(vnetId, idIpv4_2, "host_b02")
				Expect(err).NotTo(HaveOccurred())
				fmt.Printf("Allocated IP: %s/%d\n", ip, mask)
			})

			It("IPアドレスの割り当て #5", func() {
				ip, mask, err := v.AllocateIP(vnetId, idIpv4_2, "host_b03")
				Expect(err).NotTo(HaveOccurred())
				fmt.Printf("Allocated IP: %s/%d\n", ip, mask)
			})
		...
		Context("IPv6アドレスの振り出し", func() {
			...
			It("IPアドレスの割り当て #1", func() {
				_, err := v.GetIpNetworkById(vnetId, idIpv6_1)
				Expect(err).NotTo(HaveOccurred())
				ip, mask, err := v.AllocateIP(vnetId, idIpv6_1, "host1")
				Expect(err).NotTo(HaveOccurred())
				fmt.Printf("Allocated IP: %s/%d\n", ip, mask)
			})
			...
			It("IPアドレスの割り当て #2", func() {
				for i := 2; i < 20; i++ {
					hostname := fmt.Sprintf("host%d", i)
					ip, mask, err := v.AllocateIP(vnetId, idIpv6_1, hostname)
					Expect(err).NotTo(HaveOccurred())
					fmt.Printf("Allocated IP: %s/%d\n", ip, mask)
				}
			})
			...
			It("IPアドレスの割り当て #3", func() {
				ip, mask, err := v.AllocateIP(vnetId, idIpv6_1, "host255")
				Expect(err).NotTo(HaveOccurred())
				fmt.Printf("Allocated IP: %s/%d\n", ip, mask)
			})
		...
		Context("ネットワークの削除", func() {
			...
			It(" 割り当てられたIPがあるIPネットワークの削除", func() {
				err = v.DeleteIpNetworkById(vnetId, idIpv4_1)
				Expect(err).To(HaveOccurred()) // 割り当てられたIPがあるため、削除できないことを期待

				err = v.DeleteIpNetworkById(vnetId, idIpv4_2)
				Expect(err).To(HaveOccurred()) // 割り当てられたIPがあるため、削除できないことを期待

				err = v.DeleteIpNetworkById(vnetId, idIpv6_1)
				Expect(err).To(HaveOccurred()) // 割り当てられたIPがあるため、削除できないことを期待
			})

			It("再びIPアドレスのリリース #1", func() {
				ips, err := v.GetAllocatedIPs(vnetId, idIpv4_1)
				Expect(err).NotTo(HaveOccurred())
				for _, ip := range ips {
			...
			It("未使用のIPネットワークの削除", func() {
				...
				err = v.DeleteIpNetworkById(vnetId, idIpv4_2)
				Expect(err).NotTo(HaveOccurred())
				_, err = v.GetIpNetworkById(vnetId, idIpv4_2)
				Expect(err).To(HaveOccurred()) // 削除されたネットワークは取得できないことを期待
