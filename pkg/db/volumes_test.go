...
var _ = Describe("Volumes", Ordered, func() {
	...
	Describe("ボリューム管理テスト", func() {
		...
		Context("基本アクセス", func() {
			...
			It("ボリュームの作成 #1", func() {
				key, err = v.CreateVolume("data01", "/var/lib/marmot/volumes/data01.qcow2", "qcow2", "data", 10)
				Expect(err).NotTo(HaveOccurred())
			...
			It("ボリュームの作成 #2", func() {
				key, err := v.CreateVolume("data02", "/var/lib/marmot/volumes/data02.qcow2", "qcow2", "data", 10)
				Expect(err).NotTo(HaveOccurred())
				fmt.Println("Created data volume with ID:", key)
			})

			It("ボリュームの作成 #3", func() {
				key, err := v.CreateVolume("data03", "/dev/mapper/vg2/datalv0100", "lvm", "data", 10)
				Expect(err).NotTo(HaveOccurred())
				fmt.Println("Created data volume with ID:", key)
			})
