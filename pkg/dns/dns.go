package dns

import (
	"errors"
	"log/slog"
	"strings"

	db "github.com/takara9/marmot/pkg/db"
)

type DnsRecord struct {
	Hostname string
	Ipv4     string
	Ttl      uint64
}

type CoreDNSClient struct {
	dbc db.Db
}

func NewCdnsEp(etcdUrl string) (*CoreDNSClient, error) {
	var c CoreDNSClient
	dbc, err := db.NewEtcdEp(etcdUrl)
	if err != nil {
		slog.Error("faild to create endpoint of etcd database", "err", err)
		return nil, err
	}
	c.dbc = dbc

	return &c, nil
}

// Check DNS record param
func checkParam(rec DnsRecord) error {
	if len(rec.Hostname) == 0 {
		return errors.New("must set server name ")
	}

	return nil
}

func checkParam2(rec DnsRecord) error {
	if len(rec.Ipv4) == 0 {
		return errors.New("must set IP address")
	}

	return nil
}

// Make etcd path
func convertEtcdPath(Hostname string) (string, error) {
	h := strings.Split(Hostname, ".")
	if len(h) < 3 {
		return "", errors.New("must set a hostname and domain")
	}

	var etcdpath string
	for i := len(h) - 1; i >= 0; i-- {
		etcdpath = etcdpath + "/" + h[i]
	}

	return etcdpath, nil
}

// 登録
func (c *CoreDNSClient) Add(rec DnsRecord) error {
	err := checkParam(rec)
	if err != nil {
		slog.Error("", "err", err)
		return err
	}

	err = checkParam2(rec)
	if err != nil {
		slog.Error("", "err", err)
		return err
	}

	path, err := convertEtcdPath(rec.Hostname)
	if err != nil {
		slog.Error("", "err", err)
		return err
	}

	// Create JSON value
	var ent db.DNSEntry
	ent.Host = rec.Ipv4
	ent.Ttl = rec.Ttl

	// Add etcd
	path = "/skydns" + path
	err = c.dbc.PutDataEtcd(path, &ent)
	if err != nil {
		slog.Error("", "err", err)
		return err
	}

	return nil
}

// 取得
func (c *CoreDNSClient) Get(rec DnsRecord) (db.DNSEntry, error) {
	var d db.DNSEntry
	err := checkParam(rec)
	if err != nil {
		slog.Error("", "err", err)
		return d, err
	}

	path, err := convertEtcdPath(rec.Hostname)
	if err != nil {
		slog.Error("", "err", err)
		return d, err
	}

	// Get etcd
	path = "/skydns" + path
	rslt, err := c.dbc.GetEtcdByKey(path)
	if err != nil {
		slog.Error("", "err", err)
		return rslt, err
	}

	return rslt, err
}

// 削除
func (c *CoreDNSClient) Del(rec DnsRecord) error {
	err := checkParam(rec)
	if err != nil {
		slog.Error("", "err", err)
		return err
	}

	path, err := convertEtcdPath(rec.Hostname)
	if err != nil {
		slog.Error("", "err", err)
		return err
	}

	// Create JSON value
	var ent db.DNSEntry
	ent.Host = rec.Ipv4
	ent.Ttl = rec.Ttl

	// Add etcd
	path = "/skydns" + path
	err = c.dbc.DelByKey(path)
	if err != nil {
		slog.Error("", "err", err)
		return err
	}

	return nil
}
