...
var _ = Describe("ServerImageCopyingTest", Ordered, func() {
	...
	AfterAll(func(ctx0 SpecContext) {
		...
		_ = exec.Command("lvremove", "-y", "vg1/osimage-"+createdImageId1).Run()
		imageVolPath := fmt.Sprintf("/var/lib/marmot/images/%s", createdImageId1)
		_ = os.RemoveAll(imageVolPath)
		...
		_ = exec.Command("lvremove", "-y", "vg1/osimage-"+createdImageId2).Run()
	...
	Context("QCOW2 仮想サーバー起動と終了", func() {
		...
		It("仮想サーバーqcow2 起動", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			vmId[0], err = marmotServer.Ma.CreateServerManage(api.ServerID(vm))
			Expect(err).NotTo(HaveOccurred())
	...
	Context("イメージ取得のテスト", func() {
		It("仮想サーバー起動 boot + data x2 qcow2", func() {
			...
			vm, err := marmotServer.Ma.Db.MakeServerEntry(virtualServer)
			Expect(err).NotTo(HaveOccurred())
			vmId[1], err = marmotServer.Ma.CreateServerManage(api.ServerID(vm))
			Expect(err).NotTo(HaveOccurred())
	...
	Context("image-1(qcow2) からの起動テスト", func() {
		...
		It("qcow2イメージからの仮想サーバー起動", func() {
			...
			vm, err := marmotServer.Ma.Db.MakeServerEntry(virtualServer)
			Expect(err).NotTo(HaveOccurred())
			vmId[2], err = marmotServer.Ma.CreateServerManage(api.ServerID(vm))
			Expect(err).NotTo(HaveOccurred())
	...
	Context("LV仮想サーバー", func() {
		It("起動", func() {
			...
			vm, err := marmotServer.Ma.Db.MakeServerEntry(virtualServer)
			Expect(err).NotTo(HaveOccurred())
			vmId[3], err = marmotServer.Ma.CreateServerManage(api.ServerID(vm))
			Expect(err).NotTo(HaveOccurred())
	...
	Context("image-2(LV) からの起動テスト", func() {
		...
		It("LV仮想サーバー起動", func() {
			...
			vm, err := marmotServer.Ma.Db.MakeServerEntry(virtualServer)
			Expect(err).NotTo(HaveOccurred())
			vmId[4], err = marmotServer.Ma.CreateServerManage(api.ServerID(vm))
			Expect(err).NotTo(HaveOccurred())
		...
		It("仮想サーバーの削除", func() {
			Expect(marmotServer.Ma.DeleteServerByIdManage(vmId[4])).NotTo(HaveOccurred())
			//Expect(err).NotTo(HaveOccurred())
		})
	})
	...
	/*
		Context("LVのデータディスクが複数存在する仮想サーバーの起動と終了のテスト", func() {
			...
			It("仮想サーバー生成:bootはlv で最小構成", func() {
				...
				Expect(err).NotTo(HaveOccurred())
				id, err = marmotServer.Ma.CreateServer2(api.ServerID(vm))
